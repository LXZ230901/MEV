...
//  }
  ...
  public static @Nonnull CommunityContext fromEnvironment(Environment environment) {
    CommunitySet inputCommunitySet;
    if (environment.getOriginalMessage().getAttribute() instanceof BgpAttribute)
    {
      BgpAttribute attribute = (BgpAttribute) environment.getOriginalMessage().getAttribute();
      inputCommunitySet = CommunitySet.of(attribute.getCommunity());
    } else if (environment.getOriginalMessage().getAttribute() instanceof EvpnAttribute) {
      EvpnAttribute attribute = (EvpnAttribute) environment.getOriginalMessage().getAttribute();
      inputCommunitySet = CommunitySet.of(attribute.getCommunity());
    } else {
      inputCommunitySet = CommunitySet.empty();
    }
//    CommunitySet inputCommunitySet = environment.getOriginalMessage().getAttribute()
//    if (environment.getUseOutputAttributes()
//            && environment.getOutputRoute() instanceof HasReadableCommunities) {
//      HasReadableCommunities outputRoute = (HasReadableCommunities) environment.getOutputRoute();
//      inputCommunitySet = outputRoute.getCommunities();
