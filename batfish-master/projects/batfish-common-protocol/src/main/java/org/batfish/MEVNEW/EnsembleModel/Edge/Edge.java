...
public class Edge {
    ...
    public OperationAnswer processMessage(Message.Builder message, boolean maintainReflector)
    {
        ...
        if (_srcNode.getNodeType().equals(_dstNode.getNodeType()))
        {
            ...
            _srcNode.attributeMessage(outOperationAnswer.getMessage());
            ...
            if (_srcNode.getNodeType().equals(NodeType.BGP) && _dstNode.getNodeType().equals(NodeType.EVPN))
            {
                message.addVisitNode(_srcNode.getID());
                BgpAttribute inAttribute = (BgpAttribute) message.getAttribute();
                message.setAttribute(inAttribute.toEvpnAttribute());
                OperationAnswer outOperationAnswer = this._outOperation.processMessage(message);
                if (!maintainReflector)
                {
                    outOperationAnswer.getMessage().setReflector(false);
                ...
                return inOperationAnswer;
            } else if (_srcNode.getNodeType().equals(NodeType.EVPN) && _dstNode.getNodeType().equals(NodeType.BGP))
            {
                message.addVisitNode(_srcNode.getID());
                OperationAnswer outOperationAnswer = this._outOperation.processMessage(message);
                if (!maintainReflector)
                ...
                EvpnAttribute inAttribute = (EvpnAttribute) inOperationAnswer.getMessage().getAttribute();
                inOperationAnswer.getMessage().setAttribute(inAttribute.toBgpAttribute());
                return inOperationAnswer;
            }
        }
        return new OperationAnswer();
    }
