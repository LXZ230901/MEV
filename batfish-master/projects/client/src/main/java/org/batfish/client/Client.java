...
public class Client extends AbstractClient implements IClient {
  ...
  private boolean computeControlPlane(
          @Nullable FileWriter outWriter, List<String> options, List<String> parameters) {
    if (!isValidArgument(options, parameters, 0, 0, 0, Command.COMPUTE_CONTROL_PLANE)) {
      return false;
    }
    if (!isSetTestrig() || !isSetContainer(true)) {
      return false;
    ...
    // generate the data plane
    WorkItem wItemGenDp =
            WorkItemBuilder.getWorkItemComputeControlPlane(_currContainerName, _currTestrig);

    return execute(wItemGenDp, outWriter);
  }

  private boolean verifyAllReachability(
          @Nullable FileWriter outWriter, List<String> options, List<String> parameters) {
    if (!isValidArgument(options, parameters, 0, 0, 0, Command.VERIFY_ALL_REACHABILITY)) {
      return false;
    }
    if (!isSetTestrig() || !isSetContainer(true)) {
      return false;
    ...
    // generate the data plane
    WorkItem wItemGenDp =
            WorkItemBuilder.getWorkItemVerifyAllReachability(_currContainerName, _currTestrig);
  ...
  private boolean verifySingleReachability(
          @Nullable FileWriter outWriter, List<String> options, List<String> parameters) {
    if (!isValidArgument(options, parameters, 0, 5, 5, Command.VERIFY_ALL_REACHABILITY)) {
      return false;
    }
    if (!isSetTestrig() || !isSetContainer(true)) {
      return false;
    ...
    // generate the data plane
    WorkItem wItemGenDp =
            WorkItemBuilder.getWorkItemVerifySingleReachability(_currContainerName, _currTestrig, parameters);


    return execute(wItemGenDp, outWriter);
  }

  private boolean computeViolationTree(
          @Nullable FileWriter outWriter, List<String> options, List<String> parameters) {
    if (!isValidArgument(options, parameters, 0, 0, 0, Command.COMPUTE_VIOLATION_TREE)) {
      return false;
    }
    if (!isSetTestrig() || !isSetContainer(true)) {
      return false;
    ...
    // generate the data plane
    WorkItem wItemGenDp =
            WorkItemBuilder.getWorkItemComputeViolationTree(_currContainerName, _currTestrig);
  ...
  private boolean getViolationTree(
          @Nullable FileWriter outWriter, List<String> options, List<String> parameters) {
    if (!isValidArgument(options, parameters, 0, 5, 5, Command.GET_VIOLATION_TREE)) {
      return false;
    }
    if (!isSetTestrig() || !isSetContainer(true)) {
      return false;
    ...
    // generate the data plane
    WorkItem wItemGenDp =
            WorkItemBuilder.getWorkItemGetViolationTree(_currContainerName, _currTestrig, parameters);
  ...
  private boolean processCommand(
    ...
    switch (command) {
      ...
      case COMPUTE_CONTROL_PLANE:
        return computeControlPlane(outWriter, options, parameters);
      ...
      case VERIFY_ALL_REACHABILITY:
        return verifyAllReachability(outWriter, options, parameters);

      case VERIFY_SINGLE_REACHABILITY:
        return verifySingleReachability(outWriter, options, parameters);
      case COMPUTE_VIOLATION_TREE:
        return computeViolationTree(outWriter, options, parameters);
      case GET_VIOLATION_TREE:
        return getViolationTree(outWriter, options, parameters);
      case EXIT:
      case QUIT:
  ...
  private void runInteractive() {
    ...
    try {
      while (!_exit) {
        try {
          System.out.println();
          String rawLine = _reader.readLine("MEV> ");
          if (rawLine == null) {
            break;
          }
